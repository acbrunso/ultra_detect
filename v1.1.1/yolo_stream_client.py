...
class VideoThread(QThread):
    ...
    def run(self):
        ...
        try:
            ...
            return
        #need to validate connectivity still
        ...
        while self._run_flag:
            ...
            while len(data) < msg_size:
                if(msg_size - len(data) < 4096):
                    data += client_socket.recv(msg_size - len(data))                #this will grab to the end of the frame.  still need to grab to the end of the data
                else:
                    data += client_socket.recv(4096)
            frame_data = data[:msg_size]                        #this is the frame data
            

            packed_extra_data_size = client_socket.recv(4)
            ...
            if(extra_data_size > 0):
                while(len(packed_extra_data) < extra_data_size):
                    if(extra_data_size - len(packed_extra_data) < 4096):
                        packed_extra_data += client_socket.recv(extra_data_size - len(packed_extra_data))
                    else:
                        packed_extra_data += client_socket.recv(4096)
...
class Window(QWidget):
    def make_image_tile(self, hostname, row, col, id):
        ...
        3  9
        '''
        self.outer_right_layout.addWidget(self.txtHosts[-1], row, col*2, 1,2, alignment=Qt.AlignTop)

        self.outer_right_layout.addWidget(self.image_labels[-1], row, col*2, 1,2)
        ...
        self.outer_right_layout.addWidget(self.btnConnects[-1], row+1, col*2)
        ...
        self.outer_right_layout.addWidget(self.btn_toggle_predictions[-1],row+1, col*2+1)
        '''
        0 -> 2    0*3+1
        1 -> 5    1*3+1
        2 -> 8    2*3+1
    ...
    def __init__(self):
        ...
        for i in range(6):
            self.outer_right_layout.setColumnMinimumWidth(i, int(self.cam_width/2))
